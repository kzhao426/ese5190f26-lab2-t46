...
使用 **VS Code + PlatformIO + AVR-GCC**，目标板为 **ATmega328PB Xplained Mini**。代码使用寄存器级 C，未启用 Arduino 框架。当前 `src/main.c` 是 Part C 的可编译版本；Part B 提交文件仍保存在仓库根目录的 `lab2c1.c`。
...
4. 用可传输数据的 USB 线连接开发板，点击 **Upload**。Part C 烧录后，打开 9600 baud 串口监视器查看 `DOT`、`DASH` 和 `SPACE`。
...
当前 Part C 程序通过串口发送计时结果。课程 [avr-printf](https://github.com/upenn-embedded/avr-printf) 的默认配置是 **9600 baud、8 数据位、无校验、2 停止位（8N2）**。这台电脑已安装 VS Code **Serial Monitor** 扩展，可在其界面选择这些参数。
